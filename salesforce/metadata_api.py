...
from oauth import SalesforceOAuth2
...
class BaseMetadataApiCall(object):
    ...
    def _build_endpoint_url(self):
        ...
        org_id = self.cumulusci.oauth['id'].split('/')[-2]
        # If "My Domain" is configured in the org, the instance_url needs to be
        # parsed differently
        instance_url = self.cumulusci.oauth['instance_url']
    ...
    def _call_mdapi(self, headers, envelope, refresh=None):
        ...
        session_id = self.cumulusci.oauth['access_token']
    ...
    def _handle_soap_error(self, headers, envelope, refresh, response):
        ...
        if faultcode == 'sf:INVALID_SESSION_ID' and self.cumulusci.oauth and self.cumulusci.oauth['refresh_token']:
            # Attempt to refresh token and recall request
            sandbox = self.cumulusci.oauth.get('sandbox', False)
            sf = SalesforceOAuth2(
                self.client_id, self.client_secret, self.callback_url, sandbox=sandbox)
            refresh_response = sf.refresh_token(
                self.cumulusci.oauth['refresh_token'])
            if refresh_response.get('access_token', None):
                # Set the new token in the session
                self.cumulusci.oauth.update(refresh_response)
                if refresh:
    ...
    def _set_status(self, status, log=None, level=None):
        if not level:
            level = 'info'
        logger = getattr(self.cumulusci.logger, level)
        self.status = status
...
class ApiRetrieveUnpackaged(BaseMetadataApiCall):
    ...
    def __init__(self, cumulusci):
        super(ApiRetrieveUnpackaged, self).__init__(cumulusci)
...
class ApiRetrieveInstalledPackages(BaseMetadataApiCall):
    ...
    def __init__(self, cumulusci):
        super(ApiRetrieveInstalledPackages, self).__init__(cumulusci)
        self.packages = []
...
class ApiRetrievePackaged(BaseMetadataApiCall):
    ...
    def __init__(self, cumulusci, package_name):
        super(ApiRetrievePackaged, self).__init__(cumulusci)
...
class ApiDeploy(BaseMetadataApiCall):
    ...
    def __init__(self, cumulusci, package_zip, purge_on_delete=True):
        super(ApiDeploy, self).__init__(cumulusci)
    ...
    def _set_purge_on_delete(self, purge_on_delete):
        ...
        # not allowed
        org_type = self.cumulusci.oauth.get('org_type')
...
class ApiInstallVersion(ApiDeploy):
    ...
    def __init__(self, cumulusci, version, purge_on_delete=False):
        ...
        super(ApiInstallVersion, self).__init__(
            cumulusci, self.package_zip, purge_on_delete)


class ApiUninstallVersion(ApiDeploy):

    def __init__(self, cumulusci, version, purge_on_delete=True):
        self.version = version
...
class ApiListMetadata(BaseMetadataApiCall):
    ...
    def __init__(self, cumulusci, metadata_type, metadata):
        super(ApiListMetadata, self).__init__(cumulusci)
